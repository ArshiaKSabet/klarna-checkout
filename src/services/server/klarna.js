...
function getKlarnaAuth() {
	...
	const stringToBuffer = username + ':' + password;
	const auth = 'Basic ' + Buffer.from(stringToBuffer).toString('base64');
	return auth;
}
function formatProduct(cartItem) {
...
async function createOrder(products) {
	// const newArray = products.map(formatProduct);
